#!/usr/bin/python
#
# This application will output assembled coded for the UMD 24 bit RISC achitecture
#
# This will fill 1024(still need to implement check) 24 bit words in the mInstruction Memory
#
# Version: 1.0
#
...
def Assembler(INPUT,LABELS,Binary):
	...
	for Line in INPUT_LINES:
		...
		if opcode in RR_TYPE:
			RA = args[1]
			RB = args[2]
			OPDATA = (OPCODES[opcode]<<20| REGISTERS[RA]<<16| REGISTERS[RB]<<12)
			if Binary == True:
		...
		if opcode in I_TYPE:
			RA = args[1]
			IMMED = args[2]
			OPDATA = (OPCODES[opcode]<<20| REGISTERS[RA]<<16 | int(IMMED))
			if Binary == True:
				OUTPUT+= "{0:b}".format(OPDATA) + '\n'
			else:
				OUTPUT+= '%06X' % OPDATA + '\n'
		...
		if opcode in D_TYPE:
			...
			IMMED = args[3]
			ID = ID_TYPE[opcode]
			OPDATA = (OPCODES[opcode]<<20 | REGISTERS[RA]<<16 | SHADOW_REG[RS]<<14 | ID << 12 | int(IMMED))
			if Binary == True:
				OUTPUT+= "{0:b}".format(OPDATA) + '\n'
			else:
				OUTPUT+= '%06X' % OPDATA + '\n'
		...
		if opcode in B_TYPE:
			MASK = args[1]
			IMMED = args[2]
			OPDATA = (OPCODES[opcode]<<20 | REGISTERS[RA]<<16 | REGISTERS[RB])
			if Binary == True:
				OUTPUT+= "{0:b}".format(OPDATA) + '\n'
			else:
				OUTPUT+= '%06X' % OPDATA + '\n'
...
def main () :
	...
	if args.verbose == True:
		if args.binary == True:
			...
			print("[OP][RA][   IMMED 16   ]:  I [5,6,7,8,9,A,D,E]")
			print("[OP][RA]RSID[ IMMED 12 ]:  D [B,C]")
			print("[OP][MS][   IMMED 16   ]:  B [F]")
			print(OUTPUT)
